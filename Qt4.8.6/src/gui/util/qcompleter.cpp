...
static QHash< QCompleterPrivate *, Qt::MatchFlags> s_filterModeHash = QHash< QCompleterPrivate *, Qt::MatchFlags>();
...
void QCompletionModel::createEngine()
{
    ...
    Qt::MatchFlags t_filterMode = s_filterModeHash.value( c, Qt::MatchStartsWith );
    if (t_filterMode == Qt::MatchStartsWith) {
        switch (c->sorting) {
...
{
    Qt::MatchFlags t_filterMode = s_filterModeHash.value( c, Qt::MatchStartsWith );
    if (t_filterMode == Qt::MatchEndsWith)
        return;
...
QAbstractItemView *QCompleter::popup() const
{
    ...
    if (!d->popup && completionMode() != QCompleter::InlineCompletion) {
        ...
        listView->setObjectName( "PopupListView" );
...
void InoCompleter::setFilterMode(Qt::MatchFlags filterMode)
{    
    Q_D(QCompleter);
    Qt::MatchFlags t_filterMode = s_filterModeHash.value( d, Qt::MatchStartsWith );
    if (t_filterMode == filterMode)
        return;
    ...
    s_filterModeHash.insert( d, filterMode);
    d->proxy->createEngine();
    d->proxy->invalidate();
}

Qt::MatchFlags InoCompleter::filterMode() const
{
    Q_D(const QCompleter);
    Qt::MatchFlags t_filterMode = s_filterModeHash.value( const_cast<QCompleterPrivate *>(d), Qt::MatchStartsWith );
    return t_filterMode;
}
