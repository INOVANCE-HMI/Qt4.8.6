...
make distclean
make confclean
echo yes |./configure -prefix /qteFull/4.8.6 \
-embedded arm -platform qws/linux-x86-g++ -xplatform qws/linux-arm-gnueabihf-g++ \
-opensource  -fast \
-no-largefile -no-system-proxies -no-exceptions -no-stl -qt-sql-sqlite -no-qt3support \
...
cp -rf $QTDIR/include/ /qteFull/4.8.6/
cp -rf $QTDIR/src/ /qteFull/4.8.6/
tar -zcvf /qteFull/4.8.6.tar.gz /qteFull/4.8.6
