...
static const int MAX_SIZE = (1024*768);
static quint32 sg_dbuf[ MAX_SIZE ];
                                                 ...
                                                 DST *dest, int dstride)
{
	...
	//将屏幕分成拼贴块(tiles)，大于30块使用双缓冲，解决大块刷的时候有明显过渡
	bool doubleBuffer = ( numTilesX + numTilesY > 30 ); 

	int startx = w - 1;

	//计算偏移 将原来循环中的乘法改成加减法，每次循环做乘法效率低下
	const int d_stepx = dstride / sizeof(DST);	//目标数据偏移
	const int s_stepy = tileSize * sstride;		//源数据偏移
	const int d_stepb = tileSize * dstride;		//块偏移
	char* d_block = (char*)sg_dbuf;				//目标块地址

    for (int tx = 0; tx < numTilesX; ++tx, startx -= tileSize ) {
        const int stopx = qMax(startx - tileSize, 0);

		int starty = 0;
		DST *d_start = (DST*)d_block;
		const char *s_start = (const char*)(src + startx);
		
        for (int ty = 0; ty < numTilesY; ++ty, starty += tileSize ) {  //外两层循环块
            const int stopy = qMin(starty + tileSize, h);
			DST *d_buf = d_start;
			const char *s_buf = s_start;
            for (int x = startx; x >= stopx; --x ) {
				DST *d_fb = dest + ( d_buf - (DST*)sg_dbuf );
				DST *d = ( doubleBuffer ? d_buf : d_fb );
                const char *s = s_buf;
                for (int y = starty; y < stopy; ++y) {
                    *d++ = *(const DST*)(s); 	//原来使用qt_colorConvert，效率低下，改为直接拷贝
                    s += sstride;
                }
				d_buf += d_stepx;
				s_buf -= sizeof(DST);
	...
	/*
	 *复制缓冲，这里可以继续优化，改为获取到屏幕大小，然后直接拷贝整个屏幕
	 *但考虑有多屏的情况，在这个函数里不知道是哪个屏幕，大小也不能确定，暂时不修改
	 */
	 
	startx = w - 1;
	d_block = (char*)sg_dbuf;
 	for (int tx = 0; tx < numTilesX; ++tx, startx -= tileSize ) {
        const int stopx = qMax(startx - tileSize, 0);

		int starty = 0;
        for (int ty = 0; ty < numTilesY; ++ty, starty += tileSize ) {
            const int stopy = qMin(starty + tileSize, h);

			DST *d_buf = (DST*)d_block + starty;
            for (int x = startx; x >= stopx; --x ) {
				DST *d_fb = dest + ( d_buf - (DST*)sg_dbuf );
				memcpy( d_fb, d_buf, ( stopy - starty ) * sizeof(DST) );
				d_buf += d_stepx;
            }
        }
		d_block += d_stepb;
